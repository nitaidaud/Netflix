...
export class MovieService implements IMovieService {
  ...
  async getTopMovies() {
    ...
    const {data} = await tmbd.get<ITrailerResponse | null>(`/movie/${Id}/videos`);
  ...
  async getMoviesByPage(page?: number) {
    const res = await tmbd.get<IBaseResponse>(`/discover/movie`, { params: { page } });
    return res.data.results;
  }
  ...
  async getRomanceMovies(): Promise<IBaseMovie[]> {
    const res = await tmbd.get<IBaseResponse>(`/discover/movie?with_genres=10749`);
    return res.data.results;
  }

  async getKidsMovies(): Promise<IBaseMovie[]> {
    const res = await tmbd.get<IBaseResponse>(`/discover/movie?with_genres=10762`);
    return res.data.results;
  }
